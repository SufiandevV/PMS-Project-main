...
const Tasks = ({ updateState, showNotification }) => {
  ...
  const update = async (updatedData) => {
    try {
      ...
      const { data } = await axios.put(
        "http://localhost:3000/task/updateTask",
        {
          taskId,
          title: taskTitle,
          description: taskDes,
      ...
      setTeams((prevTeams) => {
        const updatedIndex = prevTeams.findIndex(
          (Team) => Team.taskId === updatedData.taskId
        );
        if (updatedIndex !== -1) {
  ...
  const getInsProjects = async (userId) => {
    try {
      ...
      const { data } = await axios.get(
        "http://localhost:3000/project/getInsProjects",
        {
          params: {
            instructorId: userId,
          },
  ...
  const getTeamByProjectId = async (projectId) => {
    try {
      const { data } = await axios.get(
        "http://localhost:3000/team/getTeamByProjectId",
        {
          params: {
            projectId: projectId,
          },
  ...
  const deleteTeam = async (taskId) => {
    try {
      ...
      setTeams((prevTeams) =>
        prevTeams.filter((team) => team.taskId !== taskId)
      );
    } catch (error) {
  ...
  return (
    <>
      <div className="w-full h-full  p-4 pt-8 bg-red text-green-700">
        <div className="app">
          {loading ? (
            <div className="flex ps-48 items-center justify-center h-screen">
              <div className="w-16 h-16  border-4 border-dashed rounded-full animate-spin border-violet-400"></div>
            </div>
          ) : (
            <div className="data-container">
              <div
                className={`className="h-screen  fade-in w-screen flex  justify-end ${
                  showNotification ? "blurr -z-50" : ""
                }`}
              >
                {isEditModalOpen && (
                  <div className="modal-container  flex items-center justify-center z-100">
                    ...
                    <div className="flex flex-col w-form gap-2 p-6 rounded-md shadow-md bg-white opacity-100 text-black">
                      ...
                      <div className="flex  mt-6">
                        <button
                          className="px-6 py-2 hover:bg-gray-300 hover:shadow-sm hover-effect rounded-sm shadow-sm bg-gray-200 text-black"
                  ...
                  <div className="modal-container  flex items-center justify-center z-100">
                    ...
                    <div className="flex flex-col w-form gap-2 p-6 rounded-md shadow-md bg-white opacity-100 text-black">
                      ...
                      <div className="flex justify-end mt-6">
                        <button
                          className="px-6 py-2 hover:bg-gray-300 hover:shadow-dm hover-effect rounded-sm shadow-sm bg-gray-200 text-black"
                  ...
                  <div className="modal-container flex items-center justify-center z-100">
                    ...
                    <div className="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md bg-white opacity-100 text-black">
                      ...
                      <div className="flex flex-col justify-end gap-3 mt-6 sm:flex-row">
                        <button
                          className="px-6 py-2 mr-5 hover:bg-gray-300 hover:shadow-sm hover-effect rounded-sm shadow-sm bg-gray-200 text-black"
                          onClick={handleCloseTeamModal}
                        >
                          Close
                        </button>
                        <button
                          className="px-6 py-2 hover:bg-red-600 hover:shadow-md hover-effect rounded-sm shadow-sm bg-red-500 text-white"
                          onClick={() => {
                            handleDeleteAction();
                            deleteTeam(selectedTeamIdForDelete);
                          }}
                        >
                          Delete Team
                        </button>
                ...
                <div className={`h-full w-full flex ${contentClassName}`}>
                  <div className=" w-full">
                    <nav className="ml-4 text-blue-700 w-full ml-4 dark:text-blue-700">
                    ...
                    <div className="container p-2 sm:p-4 text-blue-700">
                          ...
                          <div className="w-1/4 me-7">
                            <ul className="list-group shadow-lg">
                              {insProjects.map((project, index) => (
                                <li
                                  key={index}
                                  className={`list-group-item bg-white text-blue-700 cursor-pointer hover:bg-red-500 hover:text-white text-sm list-none border border-gray-200 hover-effect p-3 ${
                                    project.value === selectedProjectId
                                      ? "selected font-semibold"
                                      : ""
                                  }`}
                                  onClick={() =>
                                    handleProjectClick(project.value)
                                  }
                                >
                                  {project.label}
                                </li>
                              ...
                              <tbody>
                                ...
                                data.length > 0 ? (
                                  data.map((team, index) => (
                                    ...
                                    >
                                      <td className="border border-gray-300 bg-white px-4 py-2">
                                        <div
                                          key={index}
                                        >{`${team.firstName} ${team.lastName}`}</div>
                                      </td>
                                      <td className="p-3 border border-gray-300 text-center">
                                        <span
                                          className="px-3 py-2 text-white rounded-md bg-blue-700 cursor-pointer hover:bg-indigo-600 hover:shadow-md hover-effect"
                                          onClick={() =>
                                            handleAddClick(team.teamMemberId)
                                          }
                                        >
                                  ...
                                  <tr>
                                    <td
                                      colSpan="2"
                                      className="text-left mt-4 ms-3 text-red-500"
                                    >
                                      No Team data yet.
                                    </td>
                                  </tr>
