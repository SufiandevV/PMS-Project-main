...
const Teams = () => {
  ...
  const handleViewClick = async (teamId) => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:3000/team/getTeamMembers",
        {
          params: {
            teamId: teamId,
          },
  ...
  const getAllTeamsMember = async () => {
    ...
    try {
      const { data } = await axios.get(
        "http://localhost:3000/team/getAllMembers",
        {
          params: {
      ...
      const filteredMembers = data.response.filter(
  ...
  const getAllProjects = async () => {
    try {
      ...
      const { data } = await axios.get(
        "http://localhost:3000/project/getAllProjects",
        {
          params: {
            instructorId: userId,
          },
  ...
  const handleAddTeam = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3000/team/createTeam",
        newTeamData
      );
  ...
  const update = async (updatedData) => {
    try {
      ...
      setTeams((prevTeams) => {
        const updatedIndex = prevTeams.findIndex(
          (team) => team.teamsId === updatedData.teamsId
        );
        if (updatedIndex !== -1) {
  ...
  const getAllTeams = async () => {
    try {
      ...
      const { data: teamsData } = await axios.get(
        "http://localhost:3000/team/getAllTeams",
        {
          params: {
            instructorId: userId,
          },
      ...
      if (teamsData.response) {
        ...
        const projectsMap = new Map(
          projectsData.response.map((project) => [
            project.teamId,
  ...
  const handleBlockClick = (team) => {
    // Retrieve the blocked teams from local storage
    const blockedTeams = JSON.parse(localStorage.getItem("blockedTeams")) || [];
  
    // Add the current team to the blocked teams list
    const updatedBlockedTeams = [...blockedTeams, team.teamsId];
  
    // Store the updated blocked teams in local storage
    localStorage.setItem("blockedTeams", JSON.stringify(updatedBlockedTeams));
  
    // Simulate the removal of the team from the front end
    setTeams((prevTeams) =>
      prevTeams.filter((prevTeam) => prevTeam.teamsId !== team.teamsId)
    );
  ...
  const handleAddAction = () => {
    ...
    const isAllMembersSelected = localSelectedTeamMembers.every(
      (member) => member !== null
    );
  ...
  return (
    <>
      <div className="w-full h-full p-4 pt-8 bg-red">
        {isAddModalOpen && (
          <div className="modal-container  flex items-center justify-center z-100">
            ...
            <div className="flex flex-col w-form gap-2 p-6 rounded-md shadow-md bg-white opacity-100 text-black">
                  ...
                  <div key={index}>
                    ...
                    <Select
                      ...
                      options={memberOptions.filter(
                        (option) =>
                          !localSelectedTeamMembers.includes(option.value)
                      )}
                      onChange={(selectedOption) =>
                        handleMemberSelect(selectedOption, index)
                      }
                    />
                  </div>
                ))}{" "}
                <label htmlFor="teammember">Select team Leader</label>
                <br />
                <Select
                  className="bg-white rounded-lg mb-2 focus:outline-none text-black font-medium"
                  isSearchable={true}
                  ...
                  options={memberOptions.filter((option) =>
                    localSelectedTeamMembers.includes(option.value)
                  )}
                  onChange={(selectedOption) =>
                    handleTeamLeaderSelect(selectedOption)
                  }
                />
                {/* ... other fields */}
              </div>
              <div className="flex justify-end mt-6">
                <button
                  className="px-6 py-2 rounded-sm  hover:bg-gray-300 hover:shadow-sm hover-effect shadow-sm bg-gray-200 text-black"
                ...
                <button
                  ...
                  onClick={() => {
                    console.log(
                      ...
                      selectedTeamMembers.filter((member) => member !== null)
                    );
        ...
        {isEditModalOpen && (
          <div className="modal-container  flex items-center justify-center z-100">
            <div
              className="absolute  bg-[#efebea] opacity-50"
          ...
          <div className="modal-container flex items-center justify-center z-100">
            ...
            <div className="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md bg-white opacity-100 text-black">
              ...
              <p className="flex-1 dark:text-gray-400">
                By blocking this user, they will no longer be able to interact
                ...
                {teams.map((team, index) => (
                  <div key={index}>
                    {selectedTeamId === team.teamsId ? (
                      <button
                        className="px-6 py-2 mr-5 rounded-sm shadow-sm bg-gray-200 text-black"
                        onClick={handleCloseModal}
                      >
                        Close
                      </button>
                    ...
                    {selectedTeamId === team.teamsId ? (
                      <button
                        className="px-6 py-2 rounded-sm shadow-sm bg-red-500 text-white"
                        onClick={() => {
                          handleCloseModal(); /*blockUser(team.teamsId)*/
                        }}
                      >
                        Block
                      </button>
        ...
        <div className={`h-full w-full flex ${contentClassName}`}>
          <div className="w-full">
            <nav className="text-blue-700 w-full ml-4  dark:text-blue-700">
              <ol className="text-blue-700 h-8 flex space-x-2 dark:text-blue-700">
              ...
              <div className="overflow-x-auto w-full bg-white ">
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {teams.map((team, index) => (
                      <tr
                        key={index}
                        className="border-b border-opacity-20 border-gray-700 bg-white"
                      >
                        ...
                        <td className="p-3 border space-x-2 border-gray-300">
                          <span
                            className="px-3  py-2 text-white rounded-md bg-blue-700 cursor-pointer"
                ...
                <div className="flex justify-center space-x-4 my-4">
                  <button
                    ...
                    className="mr-2 px-3 py-1 bg-blue-700 text-white rounded"
                  >
                    <FaArrowLeft />
                  </button>
                  <button
                ...
                <div className="flex justify-center mt-10">
                  <button
                    className="justify-end rounded-sm shadow-sm bg-blue-700 text-white"
